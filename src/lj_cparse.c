...
static void cp_pragma(CPState *cp, BCLine pragmaline)
{
  cp_next(cp);
  if (cp->tok == CTOK_IDENT &&
      lj_str_fast_hash(cp->str) == H_(e79b999f,42ca3e85))  {  /* pack */
    cp_next(cp);
    cp_check(cp, '(');
    if (cp->tok == CTOK_IDENT) {
      if (lj_str_fast_hash(cp->str) == H_(738e923c,a1b65954)) {  /* push */
	if (cp->curpack < CPARSE_MAX_PACKSTACK) {
	  cp->packstack[cp->curpack+1] = cp->packstack[cp->curpack];
	  cp->curpack++;
	}
      } else if (lj_str_fast_hash(cp->str) == H_(6c71cf27,6c71cf27)) {  /* pop */
...
static void cp_decl_multi(CPState *cp)
{
  ...
  while (cp->tok != CTOK_EOF) {
	...
	continue;
		 ...
		 lj_str_fast_hash(cp->str) == H_(187aab88,fcb60b42)) { /* line */
	if (cp_next(cp) != CTOK_INTEGER) cp_err_token(cp, tok);
	cp_line(cp, hashline);
	continue;
      } else if (tok == CTOK_IDENT &&
	  lj_str_fast_hash(cp->str) == H_(f5e6b4f8,1d509107)) { /* pragma */
